...
export function Ascii({
  ...
  // The fetched art is stored with the src it came from, rather than being
  // cleared synchronously at the top of the effect when src changes. Clearing
  // it there is a setState during an effect body — a second render every time
  // the source changes — and it is unnecessary: pairing the text with its src
  // lets stale art fall out by derivation instead.
  const [fetched, setFetched] = React.useState<{ src: string; text: string } | null>(null);
  const loaded = fetched && fetched.src === src ? fetched.text : null;
  ...
  React.useEffect(() => {
    ...
    loadArt(src).then(
      (text) => {
        if (!cancelled) setFetched({ src, text });
      },
      () => {
        // Leave `fetched` alone: it is keyed by src, so a failed load simply
        // never matches and the inline fallback keeps rendering.
      },
    );
    return () => {
      cancelled = true;
